...
namespace ncrapi {
Motor::Motor(const std::string &name, const int &port, const bool &isRevase, const int &gearset) : pros::Motor(port), _name(name)

{
    sysData->i2cCheck(port, _name);
    init(clamp<int>(gearset, 0, 2, "马达齿轮"));
}
Motor::Motor(const std::string &name, const json &pragma) : pros::Motor(pragma["端口"].get<int>(), pragma["正反"].get<bool>()), _name(name)
{
