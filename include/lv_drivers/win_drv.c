...
HWND windrv_init(void) {
  ...
  lvgl_icon = (HICON)LoadImage(NULL, "lvgl_icon.bmp", IMAGE_ICON, 0, 0,
                               LR_LOADFROMFILE);
  ...
  hwnd =
      CreateWindowEx(WS_EX_CLIENTEDGE, g_szClassName, "LittlevGL Simulator",
                     WINDOW_STYLE, CW_USEDEFAULT, CW_USEDEFAULT, winrect.right,
                     winrect.bottom, NULL, NULL, GetModuleHandle(NULL), NULL);
...
static void do_register(void) {
  ...
  /* LittlevGL requires a buffer where it draw the objects. The buffer's has to
   * be greater than 1 display row
   *
   * There are three buffering configurations:
   * 1. Create ONE buffer some rows: LittlevGL will draw the display's content
   * here and writes it to your display
   * 2. Create TWO buffer some rows: LittlevGL will draw the display's content
   * to a buffer and writes it your display. You should use DMA to write the
   * buffer's content to the display. It will enable LittlevGL to draw the next
   * part of the screen to the other buffer while the data is being sent form
   * the first buffer. It makes rendering and flushing parallel.
   * 3. Create TWO screen buffer: Similar to 2) but the buffer have to be screen
   * sized. When LittlevGL is ready it will give the whole frame to display.
   * This way you only need to change the frame buffer's address instead of
   *                              copying the pixels.
   * */
  ...
  lv_disp_buf_init(&disp_buf_1, buf1_1, NULL,
                   WINDOW_HOR_RES * 10); /*Initialize the display buffer*/
